...
Requirements:
1. Add the Swift package https://github.com/gabrieltmitchell/sampler from version 0.1.0.
...
5. Call `Sampler.start()` once after the main UI is available.
...
The skill performs the same install steps automatically: detect the app structure, add the package, wire `Sampler.start()`, and recommend MCP setup if the user wants live sync.
