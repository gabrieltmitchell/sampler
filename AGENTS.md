...
If Cursor reports `EADDRINUSE 127.0.0.1:4747`, another Sampler MCP server is already running. Stop the old server or remove the Home/global Sampler MCP entry, then reload MCP. For multiple app repos, prefer project-local `.cursor/mcp.json` files generated by `sampler-mcp init`, because auto-dispatch uses the configured `--project` path.

In Cursor projects, `sampler-mcp server` auto-dispatches a local `cursor-agent --trust` run for new annotations when the Cursor CLI is installed and signed in. The widget shows one persistent status toast while the agent works, including progress such as "Making code changes..." and "Rebuilding app...", then the final resolution summary.
...
Repeated "Agent reconnecting... retry N" toasts usually mean the Cursor API is temporarily overloaded or disconnected, not that the app integration is broken.

Version labels:

- Settings sheet `Vx.y.z` is the Swift/iOS Sampler widget package version.
- `sampler-mcp --version` is the npm MCP server/tooling version.
- `sampler-mcp update` compares and reports both.
