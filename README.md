...
If `npx` itself fails before `init` can run, try `/opt/homebrew/bin/npx -y sampler-mcp@latest init` or `npm exec --yes --package=sampler-mcp@latest -- sampler-mcp init`.
...
If Cursor reports `EADDRINUSE 127.0.0.1:4747`, another Sampler MCP server is already running. Stop the old server or remove the Home/global Sampler MCP entry, then reload MCP. For multiple app repos, prefer project-local `.cursor/mcp.json` files generated by `sampler-mcp init`.

Version note: the in-app settings sheet shows the Swift/iOS widget version. `sampler-mcp --version` shows the npm MCP server version, and `sampler-mcp update` reports both.
